...
_METADATA = SourceMetadata(
    key="cryptopotato",
    display_name="CryptoPotato",
    rss_url="https://cryptopotato.com/feed/",
)


class CryptoPotatoAdapter(SourceAdapter):
    """Fetches articles from the CryptoPotato RSS feed."""
    ...
    def fetch_latest(self, limit: int = 20) -> list[RawArticle]:
        ...
        logger.info(
            "CryptoPotato fetched %d raw entries (%d valid articles)",
...
def fetch_latest(limit: int = 20) -> list[RawArticle]:
    ...
    return CryptoPotatoAdapter().fetch_latest(limit=limit)
