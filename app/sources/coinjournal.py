...
_METADATA = SourceMetadata(
    key="coinjournal",
    display_name="CoinJournal",
    rss_url="https://coinjournal.net/feed/",
)


class CoinJournalAdapter(SourceAdapter):
    """Fetches articles from the CoinJournal RSS feed."""
    ...
    def fetch_latest(self, limit: int = 20) -> list[RawArticle]:
        ...
        logger.info(
            "CoinJournal fetched %d raw entries (%d valid articles)",
...
def fetch_latest(limit: int = 20) -> list[RawArticle]:
    ...
    return CoinJournalAdapter().fetch_latest(limit=limit)
