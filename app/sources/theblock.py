...
The Block provides institutional-grade crypto news and research coverage.
Their RSS feed is reliable and includes good article summaries.
"""
...
class TheBlockAdapter(SourceAdapter):
    """Fetches articles from The Block RSS feed."""
    ...
    def fetch_latest(self, limit: int = 20) -> list[RawArticle]:
        ...
        logger.info(
            "The Block fetched %d raw entries (%d valid articles)",
...
def fetch_latest(limit: int = 20) -> list[RawArticle]:
    ...
    return TheBlockAdapter().fetch_latest(limit=limit)
