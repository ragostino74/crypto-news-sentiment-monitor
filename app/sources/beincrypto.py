...
class BeInCryptoAdapter(SourceAdapter):
    """Fetches articles from the BeInCrypto RSS feed."""
    ...
    def fetch_latest(self, limit: int = 20) -> list[RawArticle]:
        ...
        logger.info(
            "BeInCrypto fetched %d raw entries (%d valid articles)",
...
def fetch_latest(limit: int = 20) -> list[RawArticle]:
    ...
    return BeInCryptoAdapter().fetch_latest(limit=limit)
