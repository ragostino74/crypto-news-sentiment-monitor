...
Enriches content via web scraping since WordPress RSS often truncates full text.
"""
...
_METADATA = SourceMetadata(
    key="ambcrypto",
    display_name="AMBCrypto",
    rss_url="https://ambcrypto.com/feed/",
)


class AMBCryptoAdapter(SourceAdapter):
    """Fetches articles from the AMBCrypto RSS feed."""
    ...
    def fetch_latest(self, limit: int = 20) -> list[RawArticle]:
        ...
        logger.info(
            "AMBCrypto fetched %d raw entries (%d valid articles)",
...
def fetch_latest(limit: int = 20) -> list[RawArticle]:
    ...
    return AMBCryptoAdapter().fetch_latest(limit=limit)
