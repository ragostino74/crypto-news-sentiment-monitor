...
# CryptoSlate publishes via standard WordPress-style RSS.
_RSS_URL = "https://cryptoslate.com/feed/"
...
def fetch_latest(limit: int = 20) -> list[RawArticle]:
    ...
    try:
        ...
        logger.error("CryptoSlate RSS fetch failed: %s", exc)
        return []
    ...
    logger.info(
        "CryptoSlate fetched %d articles (%d parsed)", len(feed.entries), len(articles)
    )
    return articles
...
def _parse_entry(entry: dict) -> RawArticle | None:
    ...
    return entry_to_article("CryptoSlate", entry)
