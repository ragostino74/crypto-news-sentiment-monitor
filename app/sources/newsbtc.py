"""NewsBTC news source adapter.

NewsBTC is one of the oldest and most established crypto news outlets,
focusing on Bitcoin and broader cryptocurrency markets.
"""
...
_METADATA = SourceMetadata(
    key="newsbtc",
    display_name="NewsBTC",
    rss_url="https://www.newsbtc.com/feed/",
)


class NewsBTCAdapter(SourceAdapter):
    """Fetches articles from the NewsBTC RSS feed."""

    metadata = _METADATA

    def fetch_latest(self, limit: int = 20) -> list[RawArticle]:
        """Fetch latest articles from NewsBTC RSS."""
        ...
        logger.info(
            "NewsBTC fetched %d raw entries (%d valid articles)",
